...
function avantlink_settings_page() {
	...
	if ($avantlink_is_enabled) {
	    ...
	    if( isset($_POST[ $hidden_field_name ]) && $_POST[ $hidden_field_name ] == 'P' ) {
			...
			$search_results_count_opt_val = isset($_POST[ $search_results_count_data_field_name ]) ? $_POST[ $search_results_count_data_field_name ] : null;
			$display_search_term_opt_val = isset($_POST[ $display_search_term_field_name ]) ? $_POST[ $display_search_term_field_name ] : null;
...
';
	...
	//
   	...
   	//
    if (!($avantlink_is_enabled)) {
		echo wp_kses_post($strFormAuthentication);
		return;
    }
...
';
	...
	echo wp_kses($strOutputHtml, array(
            'div' => array(
                'class' => array(),
                'id' => array()
            ),
