...
<!-- <form onsubmit="insertLink();return false;" action="#"> -->
	...
	<div id="merchant_sort">Merchant :
		...
		<?php
			if (isset($_GET['merchant_id'])) {
				$merchant_id = intval(sanitize_text_field($_GET['merchant_id']));
			}
			else { $merchant_id = 0; }
	...
	<div id="search_ad_type">Ad Type :
		<select name="ad_type">
			<?php if(isset($_GET['ad_type'])) { echo '<option value="'. sanitize_text_field($_GET['ad_type']) .'">'.urldecode(sanitize_text_field($_GET['ad_type'])) .'</option>'; } ?>
	...
	<?php
	$merch_req_id = sanitize_text_field($_GET['merchant_id']);
	$ad_type_req = sanitize_text_field($_GET['ad_type']);
