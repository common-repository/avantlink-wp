<?php
/*
Plugin Name: AvantLink Ad Tools for Affiliates
Plugin URI: http://www.avantlink.com/affiliates/affiliate-tool-center/
Description: The AvantLink Ad Tools for Affiliates plugin allows quick and simple integration with a variety of affiliate marketing tools from AvantLink.The AvantLink Ad Tools for Affiliates plugin allows quick and simple integration with a variety of affiliate marketing tools from AvantLink.  Features include easy-to-use integration with Affiliate Link Encoder, Product Ad Widget, and Product Search API tools.  Requires active affiliate account with AvantLink.
Version: 1.0.19
...
function avantlink_add_pages() {
	...
	// add a menu link
	add_menu_page('AvantLink WP', 'AvantLink Tools', 'manage_options', 'avantlink-wp', 'avantlink_settings_page', plugin_dir_url( __FILE__ ) . 'optimised.svg');

	// add a new submenu under Settings:
	add_options_page(__('AvantLink Tools','avantlink-wp'), __('AvantLink Tools','avantlink-wp'), 'manage_options', 'avantlink-wp', 'avantlink_settings_page');
 ...
 */
if (is_admin() && (array_key_exists('page', $_GET) && $_GET['page'] == 'avantlink-wp')) {
...
function avantlink_is_product_search_page() {
	$strUrl = ((array_key_exists('HTTPS', $_SERVER) && $_SERVER['HTTPS'] == 'on') ? 'https://' : 'http://') . $_SERVER['SERVER_NAME'];
...
function avantlink_get_url_contents($strRequestUrl)
{
	...
	$strResponse = wp_remote_get($strRequestUrl, $args);
	$strBody = wp_remote_retrieve_body($strResponse);
	return($strBody);
}
...
class Avantlink {
	...
	function __construct() {
		...
		// Check for WP3.7 installation
		if (!defined ('IS_WP37')) {
			define('IS_WP37', version_compare($wp_version, '3.7', '>=') );
		}

		//This works only in WP3.7 or higher
		if ( IS_WP37 == FALSE) {
			add_action('admin_notices', function() {
				echo '<div id="message" class="error fade"><p><strong>' . __('Sorry, the AvantLink WP plugin works only under WordPress 3.7 or higher',"avantlinkHW") . '</strong></p></div>';
			});
			return;
		}
...
add_action( 'plugins_loaded', function() {global $avantlink; $avantlink = new Avantlink();});
?>
