...
class AvantLink_PAW_Widget extends WP_Widget {
	...
	function __construct() {
		...
		/* Create the widget. */
		parent::__construct( 'avantlink_paw_widget', __('AvantLink Product Ad Widget', 'framework'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
        ...
        // Checking to see if we are on an individual page/post
        if (is_single($id) || is_page($id)) {

            // Verifying the page or post has a PAW defined
            // If the page doesn't have a PAW defined we default to the site PAW settings
            if (trim(get_post_meta($id, '_avantlink_paw_url', true)) != '') {
