...
function avantlink_rp_inner_custom_box() {
  ...
  echo '<p><label for="avantlink_rp_keyword">' . __("Keywords/Phrases (comma separated): ", 'avantlink-wp' ) . '</label></p><p>';

  $keyword = get_post_meta($post->ID, '_avantlink_rp_keyword', true);

  echo '<input type="text" name="avantlink_rp_keyword" value="'. wp_kses_post($keyword) .'" size="25" /></p>';
  ...
  echo '<input type="text" name="avantlink_rp_nkeyword" value="'. wp_kses_post($nkeyword) .'" size="25" /></p>';
  ...
  echo '<input type="text" name="avantlink_rp_num_results" value="'. wp_kses_post($results) .'" size="10" /></p>';
